...
namespace learning_lda {
	...
	void LDASampler::DoIteration(int myid,
		int pnum,
		int tid,
		int tnum,
		LDACorpus* corpus,
		...
		int lb, ub;
		loop_static(tid, tnum, 0, corpus->size(), lb, ub);
		list<LDADocument*>::iterator iter_lb = corpus->begin();
		advance(iter_lb, lb);
		list<LDADocument*>::iterator iter_ub = corpus->begin();
		advance(iter_ub, ub);
		for (list<LDADocument*>::iterator iter = iter_lb;
		iter != iter_ub;
			++iter) {
	...
	void LDASampler::SampleNewTopicsForDocument(LDADocument* document,
		...
		document->topic_distribution_.resize(model_->num_topics());
		for (int i = 0; i < model_->num_topics(); ++i) {
			document->topic_distribution_[i] = 0;
		}
		for (LDADocument::WordOccurrenceIterator iterator(document);
		!iterator.Done();
			iterator.Next()) {
			document->topic_distribution_[iterator.Topic()] += 1;
		}
		...
		vector<int64> tmp;
		document->topic_distribution_.swap(tmp);
